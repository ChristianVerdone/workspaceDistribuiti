...
public class RumorMongeringAverageApp {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < nT; i++) {
			network[i] = new RumorMongeringAverageNode(new Info((double) 0.0, 0), network, i, 10000);
		}
		
		try {
			network[1].setPacket(new Packet(new Info(1.0d, 0), 0));
			for (int i = 0; i < nT; i++) {
				network[i].start();
			}
			Thread.sleep(3000);
			...
			for (int i = 0; i < nT; i++) {
				System.out.println(
						i + " value " + network[i].getValue().val + " la rete e composta da " + Math.round(1/(double)network[i].getValue().val)+ " nodi");
			}
